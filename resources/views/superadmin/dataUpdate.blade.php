<x-app-layout>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <h5 class="card-subtitle  mb-3">
                        Update data SKPD pegawai. Klik nomor HP untuk membuka WA. Klik NIP untuk melihat data pegawai
                        saat ini.
                    </h5>
                        ...
                        <tbody>
                            @foreach ($data as $n => $d)
                                <tr>
                                    ...
                                    <td class="bg-white"><a href="{{ route('updatedata_search', $d->nip) }}"
                                    ...
                                    <td class="bg-white">{{ $d->unit_2 }} </td>
                                    <td class="bg-white">{{ $d->unit_1 }} </td>
                                    <td class="bg-white"><a href="../storage/{{ $d->file }}"
                                            target="_blank">Lihat</a> </td>
                                    <td class="text-center bg-white">
                                        <input type="button" class="btn btn-secondary btn-sm font-11 ppx-2 ed-btn"
                                            data-quantity="{{ $d->id }}|{{ $d->pemohon }}|{{ $d->name }}|{{ $d->nip }}|{{ $d->unit_1 }}|{{ $d->unit_2 }}|{{ $d->keterangan }}"
    ...
    <script>
        $('.ed-btn').click(function() {
            ...
            var link_tolak = '/superadmin/updatedata/tolak/' + data[0];
            $('#ed_id').val(data[0]);
            $('#pemohon').val(data[1]);
            $('#ed_name').val(data[2]);
            $('#ed_nip').val(data[3]);
            $('#ed_unit_1').val(data[4]);
            $('#ed_unit_2').val(data[5]);
                    ...
                    <form class="form-horizontal validate-this px-2" novalidate method="POST"
                        ...
                        <div class="row">
                            <div class="col-sm-12 position-relative mb-2">
                                <label class="control-label">Pemohon update data</label>
                        ...
                        <div class="col-sm-12 position-relative mb-2">
                            ...
                            <input class="form-control form-control-sm" id="ed_unit_2" name=""
                                placeholder="Pilih instansi" required value="{{ old('unit') }}" readonly>
                        </div>
                        <div class="col-sm-12 position-relative mb-2">
                            <label class="control-label">Unit baru</label>
                            <input class="form-control form-control-sm" list="datalistOptions" id="ed_unit_1"
                                name="unit" placeholder="Pilih instansi" required value="{{ old('unit') }}"
                                autocomplete="off">
